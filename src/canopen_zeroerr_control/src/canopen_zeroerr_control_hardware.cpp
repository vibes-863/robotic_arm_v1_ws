...
#include "canopen_zeroerr_control/canopen_zeroerr_control_hardware.hpp"
#include <hardware_interface/types/hardware_interface_type_values.hpp>

using namespace canopen_zeroerr_control;

// auto canopen_zeroerr_control_hardware_logger = rclcpp::get_logger("canopen_zeroerr_control_hardware_interface");

hardware_interface::CallbackReturn CanopenZeroerrControlHardware::on_init(
  const hardware_interface::HardwareInfo & info)
{
  ...
  canopen_zeroerr_control_hardware_logger = rclcpp::get_logger(info_.name + "_interface");
  RCLCPP_INFO(canopen_zeroerr_control_hardware_logger, "Registering hardware interface '%s'", info_.name.c_str());

  // Check bus config is specified.
  if (info_.hardware_parameters.find("bus_config") == info_.hardware_parameters.end())
  {
    RCLCPP_ERROR(
      canopen_zeroerr_control_hardware_logger, "No bus_config parameter provided for '%s' hardware interface.",
      info_.name.c_str());
    return CallbackReturn::ERROR;
  }
  bus_config_ = info_.hardware_parameters["bus_config"];
  RCLCPP_INFO(
    canopen_zeroerr_control_hardware_logger, "'%s' has bus config: '%s'", info_.name.c_str(), bus_config_.c_str());

  // Check master config is specified.
  if (info_.hardware_parameters.find("master_config") == info_.hardware_parameters.end())
  {
    RCLCPP_ERROR(
      canopen_zeroerr_control_hardware_logger, "No master_config parameter provided for '%s' hardware interface.",
      info_.name.c_str());
    return CallbackReturn::ERROR;
  }
  master_config_ = info_.hardware_parameters["master_config"];
  RCLCPP_INFO(
    canopen_zeroerr_control_hardware_logger, "'%s' has master config: '%s'", info_.name.c_str(),
  ...
  if (info_.hardware_parameters.find("master_bin") != info_.hardware_parameters.end())
  {
    ...
    RCLCPP_INFO(
      canopen_zeroerr_control_hardware_logger, "'%s' has master bin: '%s'", info_.name.c_str(), master_bin_.c_str());
  }
  else
  {
    master_bin_ = "";
  ...
  {
    RCLCPP_ERROR(
      canopen_zeroerr_control_hardware_logger, "No can_interface_name parameter provided for '%s' hardware interface.",
      info_.name.c_str());
    return CallbackReturn::ERROR;
  }
  can_interface_ = info_.hardware_parameters["can_interface_name"];
  RCLCPP_INFO(
    canopen_zeroerr_control_hardware_logger, "'%s' has can interface: '%s'", info_.name.c_str(),
  ...
  for (auto joint : info.joints)
  {
    ...
    {
      RCLCPP_ERROR(
        canopen_zeroerr_control_hardware_logger, "Driver type '%s' not supported for joint '%s'", driver_type.c_str(),
        joint.name.c_str());
      return CallbackReturn::ERROR;
    }
  }
...
hardware_interface::CallbackReturn CanopenZeroerrControlHardware::on_configure(
  const rclcpp_lifecycle::State & previous_state)
{
  ...
  spin_thread_ = std::make_unique<std::thread>(&CanopenZeroerrControlHardware::spin, this);
  init_thread_ = std::make_unique<std::thread>(&CanopenZeroerrControlHardware::initDeviceContainer, this);
  ...
  {
    RCLCPP_ERROR(canopen_zeroerr_control_hardware_logger, "Could not join init thread!");
    return CallbackReturn::ERROR;
  }
  return CallbackReturn::SUCCESS;
}
hardware_interface::CallbackReturn CanopenZeroerrControlHardware::on_activate(
  const rclcpp_lifecycle::State & previous_state)
{
...
hardware_interface::CallbackReturn CanopenZeroerrControlHardware::on_deactivate(
  const rclcpp_lifecycle::State & previous_state)
{
  for (auto & data : robot_motor_data_)
  {
    ...
    {
      RCLCPP_ERROR(canopen_zeroerr_control_hardware_logger, "Failed to deactivate '%s'", data.joint_name.c_str());
      return CallbackReturn::FAILURE;
    }
  }
  return CallbackReturn::SUCCESS;
}
hardware_interface::CallbackReturn CanopenZeroerrControlHardware::on_cleanup(
  const rclcpp_lifecycle::State & previous_state)
{
  clean();
  return CallbackReturn::SUCCESS;
}
hardware_interface::CallbackReturn CanopenZeroerrControlHardware::on_shutdown(
  const rclcpp_lifecycle::State & previous_state)
{
  clean();
  return CallbackReturn::SUCCESS;
...
std::vector<hardware_interface::StateInterface> CanopenZeroerrControlHardware::export_state_interfaces()
{
  std::vector<hardware_interface::StateInterface> state_interfaces;

  // Iterate over joints in xacro
  for (canopen_zeroerr_control::Cia402Data & data : robot_motor_data_)
  {
...
std::vector<hardware_interface::CommandInterface> CanopenZeroerrControlHardware::export_command_interfaces()
{
...
hardware_interface::return_type CanopenZeroerrControlHardware::read(
...
hardware_interface::return_type CanopenZeroerrControlHardware::write(
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  for (canopen_zeroerr_control::Cia402Data & data : robot_motor_data_)
  {
...
hardware_interface::return_type CanopenZeroerrControlHardware::perform_command_mode_switch(
...
void CanopenZeroerrControlHardware::initDeviceContainer()
{
...
void CanopenZeroerrControlHardware::spin()
{
  executor_->spin();
  executor_->remove_node(device_container_);
  RCLCPP_INFO(device_container_->get_logger(), "Stopped spinning CanopenZeroerrControlHardware ROS2 executor");
}

void CanopenZeroerrControlHardware::clean()
{
...
PLUGINLIB_EXPORT_CLASS(canopen_zeroerr_control::CanopenZeroerrControlHardware, hardware_interface::SystemInterface)
