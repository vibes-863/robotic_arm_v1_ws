...
const std::string EEF_FRAME_ID = "end_effector_base_1";
                     ...
                     std::unique_ptr<control_msgs::msg::JointJog>& joint)
{
  ...
  if (buttons[A] || buttons[B] || buttons[X] || buttons[Y] || axes[D_PAD_X] || axes[D_PAD_Y])
  {
    ...
    joint->joint_names.push_back("taco_to_base_link");
    joint->velocities.push_back(axes[D_PAD_X]);
    joint->joint_names.push_back("shoulder_link_to_taco");
    joint->velocities.push_back(axes[D_PAD_Y]);

    // Map the diamond to the distal joints
    joint->joint_names.push_back("bevel_housing_to_elbow_link");
    joint->velocities.push_back(buttons[B] - buttons[X]);
    joint->joint_names.push_back("wrist_link_to_bevel_housing");
...
public:
    ...
    : Node("joy_to_twist_publisher", options), frame_to_publish_(BASE_FRAME_ID)
  {
    ...
    collision_pub_thread_ = std::thread([this]() {
      ...
      // Commenting out the creation of the two collision object boxes  
      /*
  ...
  void joyCB(const sensor_msgs::msg::Joy::ConstSharedPtr& msg)
  {
    ...
    else
    {
      ...
      joint_msg->header.frame_id = "elbow_link_1";
